...
The repository has 35 skills. Their frontmatter names match their directories, all required sections are present, and all local Markdown links resolve. Each description now states when to use the skill, stays below 400 characters, and fits the repository-wide discovery budget.
...
| `bio-foundation-housekeeping` | Narrowed ownership to schema-backed metadata normalization and cataloging, removed autonomous commit guidance, and fixed Pydantic field-order and revalidation examples. | Add a small executable fixture pipeline that rejects invalid metadata before Parquet and DuckDB ingestion. |
...
| `bioinformatics-project` | Moved project reproducibility into this pack, aligned it with task and memory rules, added hypothesis records and quality gates, and made its driver fail on missing stages. | Add a scaffold command that renders the bundled layout and validates a clean rerun in a temporary project. |
...
| `get-api-docs` | Updated to `@aisuite/chub`, current `search` behavior, and a verified slash-style document ID. | Record the returned documentation revision, package version, and update date in outputs so later code reviews can reproduce the lookup. |
