...
| `bio-foundation-housekeeping` | Ships a pinned Pydantic/DuckDB fixture pipeline that validates every record before atomically writing Parquet and a catalog; invalid fixtures leave both artifacts absent. | Add LinkML-to-Pydantic schema generation and cross-record foreign-key fixtures for sample, run, file, result, and provenance records. |
...
| `bioinformatics-project` | Ships a non-destructive scaffold command with a pinned-environment manifest, project records, hypothesis template, immutable-data boundary, and clean-rerun regression tests. | Add numbered-layout rendering and an optional dated first-experiment template without weakening conflict detection. |
